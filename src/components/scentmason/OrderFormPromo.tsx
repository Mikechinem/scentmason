...
type SetOption = "1" | "2" | "3" | "5";
type OilOption = "0" | "1" | "2" | "3" | "4" | "5";

// 💡 PROMO PRICING: 4 tiers only (1, 2, 3, 5 sets) — no 4-set tier on the promo page
const SET_PRICING: Record<SetOption, { label: string; price: number; save: number }> = {
  "1": { label: "1 Set", price: 27000, save: 18000 },
  "2": { label: "2 Sets", price: 50000, save: 40000 },
  "3": { label: "3 Sets", price: 73000, save: 62000 },
  "5": { label: "5 Sets", price: 122000, save: 103000 },
};

// Extra oil pricing — unchanged from normal-page OrderForm3, sold at normal price
...
export default function OrderFormPromo() {
  ...
  useEffect(() => {
    if (error) {
      const errorEl = document.getElementById("form-error-message-promo");
      if (errorEl) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const finalSets = (document.getElementById("native-hidden-sets-promo") as HTMLInputElement)?.value || sets;
    const finalOil = (document.getElementById("native-hidden-oil-promo") as HTMLInputElement)?.value || oil;
    ...
    // 💡 PROMO CHANGE: distinct fingerprint prefix so a normal-page order and a promo-page order
    // from the same phone/sets/oil combo never collide in localStorage
    const orderFingerprint = `sm_promo_order_${cleanPhone}_${finalSets}_${finalOil}`;
    ...
    const sharedEventId = `sm_promo_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
    ...
    try {
      ...
      if (typeof window !== "undefined" && (window as any).fbq) {
        (window as any).fbq("track", "Purchase", {
          content_name: "ScentMason Diffuser (Promo)",
          content_id: "scentmason_diffuser_promo",
          value: numericValue,
          currency: "NGN",
      ...
      const unifiedOrderPayload = {
        ...
        source: "promo", // 💡 PROMO ADDITION: tags this order's origin for the sheet + reporting
      };

      // Execute track calls in parallel; gracefully capture TikTok network failures
      // 💡 PROMO CHANGE: posts to the new promo-purchase route (separate Google Sheet + tagged CAPI events)
      const [metaResponse] = await Promise.all([
        fetch("/api/track/promo-purchase", {
          method: "POST",
  ...
  const fallbackSets = typeof document !== "undefined" ? (document.getElementById("native-hidden-sets-promo") as HTMLInputElement)?.value : sets;
  const fallbackOil = typeof document !== "undefined" ? (document.getElementById("native-hidden-oil-promo") as HTMLInputElement)?.value : oil;
  ...
  const successMessageText = `Hello ScentMason, I just successfully completed my Smart Fragrance Machine PROMO order form online! 
...
Please verify my delivery data details and speed up my dispatch assembly!`;
  ...
  return (
    <div className="relative z-[999999] pt-6" id="unbreakable-form-container-promo">
      <input type="hidden" id="native-hidden-sets-promo" defaultValue={sets} />
      <input type="hidden" id="native-hidden-oil-promo" defaultValue={oil} />

      <form onSubmit={handleSubmit} action="javascript:void(0)" className="bg-white text-black">
        <h2 className="text-center text-[30px] font-bold tracking-tight text-[#000000] sm:text-[22px]">
          ORDER FORM
        </h2>
          ...
          {(Object.keys(SET_PRICING) as SetOption[]).map((option) => {
            ...
            return (
              <button
                ...
                className={`native-selectable-btn-promo relative flex w-full items-center justify-between rounded-xl border-2 px-4 py-3 text-left transition-all ${
                  isActive ? "border-black bg-black/[0.04]" : "border-black/10 bg-white"
                }`}
              >
                {isBestValue && (
                  <span className="absolute -top-2.5 left-4 rounded-full bg-red-600 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">
                    Best Value
                  </span>
                )}
                <div>
        ...
        <div className="mt-3 space-y-2" id="oil-buttons-wrapper-promo">
          {(Object.keys(OIL_PRICING) as OilOption[]).map((option) => {
            ...
            return (
              <button
                ...
                className={`native-selectable-btn-promo flex w-full items-center justify-between rounded-xl border-2 px-4 py-3 text-left transition-all ${
                  isActive ? "border-black bg-black/[0.04]" : "border-black/10 bg-white"
                }`}
              >
                <p className="text-[14px] font-semibold text-black">{data.label}</p>
        ...
        <div className="mt-6 rounded-xl border-2 border-black px-4 py-4 bg-white">
          ...
          <p className="mt-1 text-[13px] font-medium text-black/70" id="native-display-summary-promo">
            {setPricing.label} · {totalOilBottles > 0 ? `${totalOilBottles} bottles` : "no extra oil"}
          </p>
          <p className="mt-2 text-[22px] font-semibold text-black" id="native-display-total-promo">
            {formatNaira(total)}
          </p>
        </div>
        ...
        {error && (
          <p id="form-error-message-promo" className="mt-4 text-[13px] font-semibold text-red-600 scroll-mt-20">
            {error}
          </p>
        )}
        ...
        >
          {!mounted ? "Loading..." : submitting ? "Sending Your Order..." : `CLAIM MY PROMO PRICE NOW — ${formatNaira(total)}`}
        </button>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        (function() {
          var container = document.getElementById("unbreakable-form-container-promo");
          if (!container) return;

          var currentPkgPrice = 27000;
          ...
          function updateDOMCalculations() {
            ...
            var displaySummary = document.getElementById("native-display-summary-promo");
            var displayTotal = document.getElementById("native-display-total-promo");
            var submitBtn = document.getElementById("native-submit-btn-promo");
            ...
            if (submitBtn && !submitBtn.innerText.includes("Sending")) {
              submitBtn.innerText = "CLAIM MY PROMO PRICE NOW — " + formatMoney(total);
            }

            var hiddenSets = document.getElementById("native-hidden-sets-promo");
            var hiddenOil = document.getElementById("native-hidden-oil-promo");
            if (hiddenSets) hiddenSets.value = currentPkgValue;
            if (hiddenOil) hiddenOil.value = currentOilValue;
          }

          container.addEventListener("click", function(e) {
            var btn = e.target.closest(".native-selectable-btn-promo");
            if (!btn) return;
            ...
            if (type === "package") {
              ...
              var wrapper = document.getElementById("package-buttons-wrapper-promo");
              if (wrapper) {
                var buttons = wrapper.getElementsByClassName("native-selectable-btn-promo");
              ...
              var wrapper = document.getElementById("oil-buttons-wrapper-promo");
              if (wrapper) {
                var buttons = wrapper.getElementsByClassName("native-selectable-btn-promo");
                for (var i = 0; i < buttons.length; i++) {
                  buttons[i].classList.remove("border-black", "bg-black/[0.04]");
