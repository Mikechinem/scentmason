    ...
    <div className="relative z-[999999] pt-6" id="unbreakable-form-container">
      ...
      <form onSubmit={handleSubmit} action="javascript:void(0)" className="bg-white text-black">
        ...
        <div className="mt-7 space-y-5">
          {[
            ["Full Name", name, setName, "e.g. Chioma Adeyemi"],
            ["Phone Number", phone, setPhone, "08012345678"],
            ["WhatsApp Number", whatsapp, setWhatsapp, "08012345678"],
          ].map(([label, value, setter, placeholder]) => (
            <div key={String(label)}>
              <label className="text-[15px] font-medium text-black/70">{String(label)}</label>
              <input
                type="tel"
                value={String(value)}
