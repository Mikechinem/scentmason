...
> = {
  "1": {
    ...
    price: 26990,
    save: 0,
    description: "1 Machine + 1 × 80ml Fragrance Oil",
  },
  "2": {
    label: "2 Sets",
    price: 49990,
    save: 3990,
    description: "2 Machines + 2 × 80ml Fragrance Oils",
  },
  "3": {
    label: "3 Sets",
    price: 72990,
    save: 7980,
    description: "3 Machines + 3 × 80ml Fragrance Oils",
  },
  "4": {
    label: "4 Sets",
    price: 94990,
    save: 12970,
    description: "4 Machines + 4 × 80ml Fragrance Oils",
  },
};
...
export default function OrderFormNewPricePremium() {
  ...
  useEffect(() => {
    if (error) {
      const errorEl = document.getElementById("premium-form-error-message");
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const orderFingerprint = `sm_new_price_premium_${cleanPhone}_${finalSets}_${finalOil}`;
    ...
    if (
      ...
      setSubmittedPackage(
        SET_PRICING[finalSets as SetOption].label
      );
      setSubmitted(true);
      return;
    }
    ...
    /*
     * New tracking namespace.
     *
     * This is intentionally different from the existing
     * OrderFormNewPrice system so the two forms remain separate.
     */
    const sharedEventId = `sm_new_price_premium_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 9)}`;

    const leadEventId = `sm_new_price_premium_lead_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 9)}`;

    const completeRegistrationEventId = `sm_new_price_premium_complete_registration_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 9)}`;
    ...
    try {
      ...
      const unifiedOrderPayload = {
        eventName: "NewPricePremiumOrder",
       ...
       * NEW ORDER ROUTE
       *
       * This is the only order endpoint used by this form.
       */
      const orderResponse = await fetch(
        "/api/track/NewPrice-Order",
        {
          method: "POST",
          headers: {
      ...
      if (!orderResponse.ok) {
        throw new Error(
          "Primary order recording engine failed"
        );
      }
      ...
      if (
        !orderResult?.success ||
        !orderResult?.orderCaptured
      ) {
        throw new Error(
          "Order was not confirmed as recorded"
        );
      }
      ...
      console.error(
        "New Price Premium order submission tracking loop exception:",
        err
      );
  ...
  const selectedPackageLabel =
    SET_PRICING[sets].label;

  const chosenOilLabel =
    OIL_PRICING[oil].label;
...
Please verify my delivery data details and speed up my dispatch assembly!`;
  ...
  return (
    <div
      className="relative z-[999999] pt-6"
      id="new-price-premium-form-container"
    >
      <input
        type="hidden"
      ...
      >
        ...
        <div className="rounded-xl border-2 border-red-600 bg-red-50 p-4">
          <p className="rounded-md border border-red-200 bg-red-50 p-4 text-[17px] font-medium leading-7 text-red-700">
            ...
            Failed deliveries cost us a lot of money.
            ...
            if you are{" "}
            <span className="font-bold">
              fully ready to receive your order
            </span>{" "}
            and{" "}
            <span className="font-bold">
              pay upon delivery
            ...
            details before dispatching. Thank you for
            respecting our business!
          </p>
        </div>
        ...
        {/* =====================================================
            PACKAGE
        ===================================================== */}
        <p className="mt-7 text-[20px] font-bold">
          Choose your package
        </p>
          ...
          {(Object.keys(
            ...
            return (
              <button
                ...
                className={`premium-selectable-btn flex w-full items-center justify-between gap-4 rounded-xl border-2 px-4 py-4 text-left transition-all ${
                  isActive
              ...
              >
                <div className="min-w-0">
                  <p className="text-[17px] font-bold text-black sm:text-[18px]">
                    {data.label}
                  </p>

                  <p className="mt-1 text-[14px] font-medium leading-5 text-black/60 sm:text-[15px]">
                    {data.description}
                  </p>

                  {data.save > 0 && (
                    <p className="mt-1 text-[14px] font-bold text-emerald-700 sm:text-[15px]">
                      Save {formatNaira(data.save)}
                    </p>
                  )}
                </div>

                <p className="shrink-0 text-[21px] font-bold text-black sm:text-[22px]">
        ...
        <p className="mt-1 text-[17px] font-medium leading-6 text-black/60">
          Each set already includes one 80ml fragrance
          oil. Add more only if you want extra fragrance
          supply.
        </p>
        ...
        >
          {(Object.keys(
            ...
            return (
              ...
              >
                <div>
                  <p className="text-[16px] font-bold text-black sm:text-[17px]">
                    {data.label}
                  </p>

                  {data.duration && (
                    <p className="mt-0.5 text-[14px] font-medium text-black/55">
                      {data.duration}
                    </p>
                  )}
                </div>
        ...
        <div className="mt-7 rounded-xl border-2 border-black bg-white px-4 py-5">
          <p className="text-[14px] font-bold uppercase tracking-wide text-black/50">
            Order Total
          </p>

          <p
            className="mt-1 text-[16px] font-semibold text-black/70"
            id="premium-display-summary"
          >
            {setPricing.label} · {setPricing.description}
            {totalOilBottles > 0
              ? ` · +${totalOilBottles} extra oil`
              : ""}
          </p>

          <p
            className="mt-2 text-[30px] font-bold text-black"
    ...
    <div key={String(label)}>
      ...
      />
        ...
        {error && (
          <p
            ...
            className="mt-5 text-[15px] font-semibold leading-6 text-red-600 scroll-mt-20"
          >
            {error}
          </p>
        )}
        ...
        <div className="mt-7 rounded-xl border border-red-200 bg-red-50/70 p-4">
          <label className="flex cursor-pointer items-start gap-3">
            ...
            <span className="text-[16px] font-bold leading-6 text-gray-800">
              I WILL ACCEPT — I am ready to receive my
              order when contacted for delivery
              confirmation.
            </span>
          </label>
        </div>
          ...
          <div className="fixed inset-0 z-[9999999] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fadeIn">
            <div className="relative w-full max-w-md transform rounded-2xl border border-black/5 bg-white p-6 text-center text-black shadow-2xl animate-scaleIn">
              ...
              <p className="mt-3 px-2 text-[16px] font-medium leading-7 text-black/70">
                ...
                faster, please inform us on WhatsApp.
              </p>
              ...
              <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50/80 p-4 text-left">
                <p className="text-[15px] font-bold leading-6 text-amber-950">
                  ⚠️ WHAT NEXT? A ScentMason customer
                  care representative will call you shortly
                  on{" "}
              ...
              <a
                ...
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-6 py-4 text-center text-[17px] font-bold text-white shadow-md transition-all hover:scale-[1.01] active:scale-100"
              >
                <svg
                  viewBox="0 0 32 32"
      ...
      <script
        dangerouslySetInnerHTML={{
          __html: `
            (function() {
              var container = document.getElementById("new-price-premium-form-container");
              if (!container) return;
              ...
              function updateDOMCalculations() {
                ...
                if (
                  ...
                  !submitBtn.innerText.includes("ORDER PLACED")
                ) {
                  submitBtn.innerText =
                    "YES I WANT THIS NOW — " +
                    formatMoney(total);
              ...
              container.addEventListener(
                ...
                function(e) {
                  ...
                  if (type === "package") {
                    ...
                    if (packageWrapper) {
                      var buttons =
                        packageWrapper.getElementsByClassName(
                          "premium-selectable-btn"
                        );
                    ...
                    if (oilWrapper) {
                      var buttons =
                        oilWrapper.getElementsByClassName(
                          "premium-selectable-btn"
                        );
