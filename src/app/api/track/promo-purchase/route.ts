...
type PurchaseRequestBody = {
  ...
  oilPrice?: number;
  total?: string | number;
  source?: string; // 💡 PROMO ADDITION: tags which page/campaign this order came from
};
...
export async function GET() {
  ...
  return NextResponse.json({
    status: "ok",
    message: "ScentMason Promo Meta CAPI Purchase & Google Sheets route is active.",
    envCheck: {
      hasDatasetId: Boolean(datasetId),
      hasAccessToken: Boolean(accessToken),
      // 💡 PROMO CHANGE: reads the dedicated promo sheet var, kept separate from the normal-page route
      hasGoogleSheetsUrl: Boolean(process.env.GOOGLE_SHEETS_PROMO_WEBHOOK_URL),
    },
  });
}
...
export async function POST(req: NextRequest) {
  try {
    const { datasetId, accessToken, graphVersion, testEventCode } = getMetaEnv();
    // 💡 PROMO CHANGE: dedicated env var so this never touches the normal page's sheet
    const googleSheetsUrl = process.env.GOOGLE_SHEETS_PROMO_WEBHOOK_URL;

    // --- Dynamic Multi-Pixel Array Definition (identical to normal route) ---
      ...
      data: [
        {
          ...
          custom_data: removeEmptyValues({
            // 💡 PROMO CHANGE: distinct content_name/content_id so Ads Manager can separate promo vs normal conversions
    ...
    if (googleSheetsUrl) {
      const sheetsPayload = {
        ...
        total: body.total || "",
        source: body.source || "promo", // 💡 PROMO ADDITION: tags this row's origin in the sheet
      };
      ...
      console.warn("Google Sheets synchronization skipped: Missing GOOGLE_SHEETS_PROMO_WEBHOOK_URL variable.");
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Promo purchase pixel captured and sheet synchronised.",
      eventId,
    });
  } catch (error) {
